...
import { Check, Sparkles, Sliders, ArrowRight, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-6xl mx-auto font-sans">
      ...
      <div className="flex justify-center mb-8">
        <div className="inline-flex p-1 rounded-xl bg-[var(--bg-surface-elevated)] border border-[var(--border-subtle)] font-mono text-xs">
          <button
            onClick={() => setViewMode('cards')}
            className={`px-4 py-2 rounded-lg transition-all ${
              viewMode === 'cards'
                ? 'bg-cyan-500/20 text-cyan-600 dark:text-cyan-300 border border-cyan-400/40 shadow-sm'
      ...
      {viewMode === 'cards' ? (
          ...
          {PRICING_TIERS.map((tier) => (
            <div
              ...
              className={`relative rounded-2xl p-6 flex flex-col justify-between transition-all duration-300 ${
                tier.highlight
                  ? 'border-2 border-cyan-500/70 bg-[var(--bg-surface-elevated)] shadow-xl lg:-translate-y-2'
                  : 'border border-[var(--border-subtle)] bg-[var(--bg-card)] hover:border-cyan-500/30'
              }`}
            >
              {tier.highlight && (
              ...
              <div>
                <div className="flex items-center justify-between text-[var(--text-muted)] font-mono text-[11px] mb-2">
                  <span>{tier.code}</span>
                  <span className="text-cyan-500 font-semibold">ALCANCE A MEDIDA</span>
                </div>

                <h3 className="text-lg font-bold text-[var(--text-primary)] mb-1">{tier.name}</h3>
                <p className="text-xs text-[var(--text-secondary)] min-h-[36px] mb-4">{tier.subtitle}</p>

                <div className="mb-6 pb-6 border-b border-[var(--border-subtle)]">
                  <span className="text-xs text-[var(--text-muted)] font-mono block">Inversión:</span>
                  <div className="flex items-baseline space-x-1">
                    <span className="text-3xl font-extrabold text-[var(--text-primary)] tracking-tight">{tier.price}</span>
                  </div>
                  <span className="text-[11px] text-[var(--text-muted)] italic block mt-1">
                    *Precio orientativo según requerimientos
                  </span>
                </div>

                <ul className="space-y-2.5 mb-6 text-xs text-[var(--text-secondary)]">
              ...
              <button
                ...
                className={`w-full py-2.5 px-4 rounded-xl font-medium text-xs transition-all flex items-center justify-center space-x-2 cursor-pointer ${
                  tier.highlight
                    ? 'bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold shadow-lg shadow-cyan-950/30'
                    : 'bg-[var(--bg-surface-elevated)] hover:bg-black/5 dark:hover:bg-white/[0.08] text-[var(--text-primary)] border border-[var(--border-subtle)]'
                }`}
              >
                <span>{tier.cta}</span>
        ...
        <div className="rounded-2xl border border-cyan-500/40 bg-[var(--bg-card)] p-6 sm:p-8 shadow-2xl relative overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-6 mb-6 border-b border-[var(--border-subtle)] gap-3">
            <div>
              <div className="flex items-center space-x-2 text-cyan-500 text-xs font-mono mb-1">
                <Sparkles className="w-3.5 h-3.5" />
                <span>COTIZADOR TÉCNICO INTERACTIVO</span>
              </div>
              <h3 className="text-xl font-bold text-[var(--text-primary)]">Configura los módulos de tu proyecto</h3>
            </div>
            <div className="text-left sm:text-right">
              <span className="text-xs font-mono text-[var(--text-muted)] block">Presupuesto Estimado</span>
              <span className="text-3xl font-mono font-extrabold text-cyan-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-6 space-y-4">
              <span className="block text-xs font-mono uppercase text-[var(--text-muted)]">
                ...
                {Object.entries(basePlans).map(([key, plan]) => (
                  <div
                    ...
                    className={`p-3.5 rounded-xl border cursor-pointer transition-all ${
                      selectedBase === key
                        ? 'border-cyan-500 bg-cyan-500/10 text-[var(--text-primary)] shadow-sm'
                        : 'border-[var(--border-subtle)] bg-[var(--bg-surface-elevated)] text-[var(--text-secondary)] hover:border-cyan-500/30'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-semibold text-sm text-[var(--text-primary)]">{plan.name}</span>
                    ...
                    <div
                      ...
                      className={`p-3.5 rounded-xl border cursor-pointer transition-all flex items-center justify-between ${
                        isActive
                          ...
                          : 'border-[var(--border-subtle)] bg-[var(--bg-surface-elevated)] text-[var(--text-secondary)] hover:border-violet-500/30'
                      }`}
                    >
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <div className="flex items-center space-x-2">
                            <span className="text-xs font-semibold text-[var(--text-primary)]">{addon.name}</span>
                            <span className="text-[10px] font-mono text-[var(--text-muted)]">{addon.code}</span>
                          </div>
                          <p className="text-[11px] text-[var(--text-secondary)]">{addon.description}</p>
                        </div>
                      </div>

                      <span className="font-mono text-xs text-violet-600 dark:text-violet-300 font-bold ml-2 shrink-0">
                        +US${addon.price}
                      </span>
                    </div>
                  );
          ...
          <div className="mt-8 pt-6 border-t border-[var(--border-subtle)] flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <button
              onClick={scrollToContact}
              className="w-full sm:w-auto px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-semibold text-xs transition-all shadow-lg shadow-cyan-950/30 flex items-center justify-center space-x-2 cursor-pointer"
            >
